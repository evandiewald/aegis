...
2. FAULT_LOCALIZATION: Next, begin the critical phase of identifying the problematic file(s), function(s)/class(es), and specific lines of code that cause the bug. Use `ls`, `search_files`, and `code_search` to narrow down to a specific file / line, then use `open_file`, `scroll_up`, and `scroll_down` to inspect the code in that specific location. NOTE: it's most efficient to search first, open the file at that line, THEN scroll as needed. Once you've identified the bug, you're ready for the next step.
...
5. CLEANUP_AND_SUBMIT: You should delete `reproduce_issue.py` using the `rm` tool so that it doesn't get submitted in the fix. Finally, use `submit` to let us know you're complete.
...
Use the provided tools to reproduce the issue, identify the bug, make the proper edits, validate the fix, and then submit.
"""
